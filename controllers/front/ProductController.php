        ...
        if (!$this->errors) {
            ...
            if ($this->product->customizable) {
                ...
                foreach ($files as $file) {
                    // customized by phinq
                    if (file_exists(_PS_UPLOAD_DIR_.$file['value'].'.pdf')) {
                        $pictures['pictures_' . $this->product->id . '_' . $file['index']] = $file['value'].'.pdf';
                    } elseif (file_exists(_PS_UPLOAD_DIR_.$file['value'].'.ai')) {
                        $pictures['pictures_' . $this->product->id . '_' . $file['index']] = $file['value'].'.ai';
                    } else {
            ...
            // customized by phinq
            $pdfUploads = $aiUploads = [];
            $site_url = Context::getContext()->shop->getBaseURL(true);
            if ($pictures) {
                foreach ($pictures as $picture) {
                    if (strpos($picture, '.pdf') !== false) {
                        $pdfUploads[] = $site_url.'upload/'.$picture;
                    } elseif (strpos($picture, '.ai') !== false) {
                        $aiUploads[] = $site_url.'upload/'.$picture;
                    }
                }
            }
