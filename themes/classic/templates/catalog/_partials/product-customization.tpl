              ...
              <li class="product-customization-item">
                ...
                {elseif $field.type == 'image'}
                  {if $field.is_customized}
                    <br>
                    {if count($pdfUploads) > 0 && in_array($field.image.small.url, $pdfUploads)}
                      <a src="{$field.image.small.url}.pdf" target="_blank" title="pdf">
                        <img src="{$site_url}img/pdf.png" width="40"/>
                      </a>
                      <a class="remove-image" href="{$field.remove_image_url}" rel="nofollow">{l s='Remove File' d='Shop.Theme.Actions'}</a>
                    {elseif count($aiUploads) > 0 && in_array($field.image.small.url, $aiUploads)}
                      <a src="{$field.image.small.url}.ai" target="_blank" title="ai">
                        <img src="{$site_url}img/ai.png" width="40"/>
                      </a>
                      <a class="remove-image" href="{$field.remove_image_url}" rel="nofollow">{l s='Remove File' d='Shop.Theme.Actions'}</a>
                    {else}
                  ...
                  <small class="float-xs-right">{l s='.png .jpg .gif, .pdf, .ai' d='Shop.Forms.Help'}</small>
                {/if}
              </li>
            {/foreach}
          </ul>
