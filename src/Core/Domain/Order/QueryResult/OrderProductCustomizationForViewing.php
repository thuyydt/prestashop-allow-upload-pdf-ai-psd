...
class OrderProductCustomizationForViewing
{
    ...
    public function __construct(int $type, string $name, string $value)
    {
        ...
        if (Product::CUSTOMIZE_FILE === $this->type) {
            if (\file_exists(_PS_UPLOAD_DIR_.$this->value.'.pdf')) {
                $this->image = _THEME_PROD_PIC_DIR_ . $this->value . '.pdf';
            } elseif (\file_exists(_PS_UPLOAD_DIR_.$this->value.'.ai')) {
                $this->image = _THEME_PROD_PIC_DIR_ . $this->value . '.ai';
            } else {
